...
App.loadPv = function() {
  const userData = getUserData();
  const bonus = userData[`Diva_PV_pts`] || 0;
  return Math.round((1 + bonus * 0.02) * 11021);
};

App.loadAttaque = function() {
  const userData = getUserData();
  const bonus = userData[`Diva_attaque_pts`] || 0;
  return Math.round((1 + bonus * 0.02) * 475);
};

App.loadDefense = function() {
  const userData = getUserData();
  const bonus = userData[`Diva_defense_pts`] || 0;
  return Math.round((1 + bonus * 0.02) * 100);
};

App.levelUp = function() {
  const userData = getUserData();
  const level = userData['Diva_Level'];
  const xp = userData['Diva_XP'];
  ...
  if (level < 11 && xp >= xpNeeded && points >= cost) {
    userData['Diva_Level'] += 1;
    userData['Diva_XP'] -= xpNeeded;
    userData.Diva_pts += 4;
...
App.afficherDonneesUtilisateur = function() {
  ...
  const userData = getUserData();
  userData.Diva_pts = userData.Diva_pts || 0;
  userData.Diva_PV_pts = userData.Diva_PV_pts || 0;
  userData.Diva_attaque_pts = userData.Diva_attaque_pts || 0;
  userData.Diva_defense_pts = userData.Diva_defense_pts || 0;
  saveUserData(userData);
  ...
  if (userData['Diva'] === 1) {
    ...
    const divaHTML = `
      <div id="Diva-info" class="character-info">
        ...
        Attaque : ${attaque}<br>
        Défense : ${defense}<br>
        Spécialité : Réduit l'attaque adverse de 25% pour 3 tours.<br>
        Rareté : rare<br>
      </div>
  ...
  console.log(userData.Diva_pts);

  if (
    userData.Diva_pts > 0 &&
    userData.Diva_PV_pts < 25 &&
    userData.Diva_attaque_pts < 20 &&
    userData.Diva_defense_pts < 20
  ) {
...
App.afficherBoutonsStats = function(userData) {
  ...
  stats.forEach((stat) => {
    const valeurStat =
      (userData[`Diva_${stat}_pts`] || 0) + (App.modificationsTemp[stat] || 0);
    let maxStat = 0;
    if (stat === 'PV') {
      maxStat = 25;
    ...
    const boutonPlusVisible =
      valeurStat < maxStat &&
      (userData.Diva_pts - App.totalPointsUtilises()) > 0;
  ...
  bubble.textContent = `Points restants : ${userData.Diva_pts - App.totalPointsUtilises()}`;
...
App.confirmerStats = function() {
  ...
  for (const stat of ["PV", "attaque", "defense"]) {
    userData[`Diva_${stat}_pts`] =
      (userData[`Diva_${stat}_pts`] || 0) + (App.modificationsTemp[stat] || 0);
  }
  userData.Diva_pts -= totalPts;
  App.modificationsTemp = {};
